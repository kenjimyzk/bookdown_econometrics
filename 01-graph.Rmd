...
点の散らばりも加えるためには `gf_jitter` も実施する.
```{r}
gf_boxplot(log(wage)~sex,fill=~sex,data=CPS85,outlier.alpha = 0) %>% gf_jitter(alpha=0.5)
```
外れ値が二重にならないように最初のオプションに`outlier.alpha = 0` を加える.
