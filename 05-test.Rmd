...
set.seed(2000)
``` 
推測統計学において, 母集団を確率変数に考えて,
標本は独立同一分布にしたがう確率変数とする.
複数の確率変数である標本の関数を統計量という.

推測統計学が統計量から母集団について何らかの判断をする.
母集団の特徴づけるものを母数という.
...
```{r}
# siml<-function(N=100,mean=0,sd=1) {
#   x<-rnorm(N,mean,sd)
#   m<-mean(x)
#   ml<-m-1.96*sd/sqrt(N)
#   mu<-m+1.96*sd/sqrt(N)
#   return(data.frame(m,ml,mu))
# }
set.seed(1234)
simulation <- do(20) * {
  N = 100
...
これを図示すると以下になる.
```{r, fig.keep="last"}
dotplot(.index~m ,data=simulation, ylab="", pch = 16, cex = 1,
        xlim=c(min(simulation$ml) - .02, max(simulation$mu) + .02))
ladd(panel.abline(v = 10, col = 2, lty = 2))
ladd(panel.segments(simulation$ml, as.numeric(y), 
                    simulation$mu, as.numeric(y), 
                    lty = 1, col = "blue"))
```
...
cor.test(pcs~cesd,data=df)
```
である. `cor.test(df$pcs,df$cesd)` でも同様の結果になる. 