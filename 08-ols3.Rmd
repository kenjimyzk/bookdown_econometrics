...
作図すると以下である.
```{r}
xyplot(log(subs)~log(citeprice),data=df)
```
...
plotFun(log(fn_w1(exp(x)))~x,add = TRUE, col = 2)
plotFun(log(fn_w2(exp(x)))~x,add = TRUE, col = 3)
```
...
```{r, fig.keep = "last"}
histogram(~tval, data = siml, xlim = c(-4,4), ylim=c(0,0.5))
plotDist("norm", add = TRUE)
```
