...
plotFun(fn(wt,hp=10)~wt,add=TRUE)
plotFun(fn(wt,hp=mhp)~wt,add=TRUE,mhp=with(mtcars,mean(hp)),col=2)
```
...
histogram(~x, data = siml,xlim = c(0,2))
```
...
```{r}
favstats(~x, data = siml)
histogram(~w, data = siml,xlim = c(-1,1))
```
...
正しい重回帰モデルの変数 $x$ の傾きの分布は以下である.
```{r}
siml <- do(1000) * lm(y~x + w,data=simdata2(100))
histogram(~x, data = siml,xlim = c(0,2))
```

重回帰モデルは不偏でなく正のバイアスが発生している.
```{r}
siml <- do(1000) * lm(y~x,data=simdata2(100))
histogram(~x, data = siml, xlim = c(1,3))
```
